...
import { Building2, Plus, Save, X, Loader, LogOut, UserPlus, Eye, EyeOff, Users, ChevronDown, ChevronRight } from "lucide-react";
...
const USER_FORM_VAZIO = { nome:"", email:"", senha:"", confirmarSenha:"", perfil:"Gestor", empresas:[] };

export default function EmpresaSelector() {
  const {
    user, userProfile, logout, isAdmin, canEdit,
    empresas, loadingEmpresas, selecionarEmpresa, criarEmpresa,
    usuarios, criarUsuario, excluirUsuario,
  } = useApp();
  ...
  const handleExcluirUser = async (u) => {
    if (u.id === user?.uid) return alert("Você não pode excluir o próprio usuário.");
      ...
      <div style={{ padding:"32px 20px", maxWidth:960, margin:"0 auto" }}>
        ...
        {isAdmin && (
          <div style={{ marginTop:32 }}>
            <button
              onClick={() => setPainelUsuarios(p => !p)}
              style={{ display:"flex", alignItems:"center", gap:10, width:"100%", background:C.white, border:`1px solid ${C.border}`, borderRadius:10, padding:"14px 18px", cursor:"pointer", marginBottom: painelUsuarios ? 0 : 0 }}
            >
            ...
            {painelUsuarios && (
              <Card style={{ borderTopLeftRadius:0, borderTopRightRadius:0, borderTop:"none", marginTop:0 }}>

                {/* Botão novo usuário */}
                <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:14 }}>
                  <Btn onClick={() => { setCriandoUser(p => !p); setUserErr(""); setUserForm(USER_FORM_VAZIO); }} color={C.navyMid} icon={<UserPlus size={13}/>}>
                    Novo Usuário
                  </Btn>
                </div>
                ...
                {criandoUser && (
                  <div style={{ background:"#f8fafc", border:`1px solid ${C.navyMid}`, borderRadius:8, padding:16, marginBottom:16 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ position:"relative" }}>
                          <input type={showPwd ? "text" : "password"} value={userForm.senha}
                      ...
                      <div>
                        ...
                        <input type={showPwd ? "text" : "password"} value={userForm.confirmarSenha}
                          onChange={e => setUserForm(p => ({ ...p, confirmarSenha:e.target.value }))}
                          placeholder="••••••"
                        ...
                        />
                        {userForm.confirmarSenha && userForm.confirmarSenha !== userForm.senha && (
                      ...
                      <div>
                        ...
                        <p style={{ fontSize:10, color:C.muted, margin:"3px 0 0" }}>
                          {userForm.perfil === "Admin" ? "Acesso total" : userForm.perfil === "SESMT" ? "Cria e edita avaliações" : "Apenas leitura"}
                        </p>
                      </div>
                    </div>
                    ...
                    {userErr && (
                      <div style={{ background:"#fee2e2", border:"1px solid #fca5a5", borderRadius:6, padding:"8px 12px", marginBottom:10, fontSize:12, color:C.red }}>{userErr}</div>
                    )}
                    ...
                    {usuarios.map(u => {
                      ...
                      return (
                        <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, background:C.bg, borderRadius:8, padding:"10px 12px" }}>
                          ...
                          {!ehEuMesmo && (
                            <button onClick={() => handleExcluirUser(u)} style={{ background:"none", border:"none", cursor:"pointer", color:C.red, padding:4 }}>
                              <X size={14}/>
                            </button>
                          )}
                        </div>
